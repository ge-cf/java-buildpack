...
module JavaBuildpack::Diagnostics
  ...
  class LoggerFactory
    ...
    def self.create_logger(app_dir)
      ...
      @@monitor.synchronize {
        @@logger = Logger.new(LogSplitter.new(File.open(log_file, 'a'), $stdout, $stderr))
      }
      ...
      @@logger.formatter = proc do |severity, date_time, prog_name, message|
        {:severity => severity,
         :content => default_formatter.call(severity, date_time, prog_name, message)}
      end
    ...
    class LogSplitter
      ...
      def write(arg)
        ...
        specific_destination = case
                               when severity == DEBUG_SEVERITY_STRING then
                                 @info
                               when severity == INFO_SEVERITY_STRING then
                                 @info
                               else
                                 @error
                               end

        specific_destination.write content
        specific_destination.flush
      end
